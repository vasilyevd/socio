...
class UGMail {
	 ...
	 */
	public function __construct(PersonUser $model, $message)
	{
		$this->_urlaction = $this->seturlaction($message);
		...
		if ($this->_swiftmail) {
			...
			$this->_message->from = array($this->_from => Yii::app()->name);
		} else {
	 ...
	 */
	protected function extractUserData(PersonUser $model)
	{
		$this->_to = "{$model->username} <{$model->email}>";
		$link = 'http://'.$_SERVER['HTTP_HOST'].Yii::app()->homeUrl.'person/person/'.$this->_urlaction;
		$full_link = $link.'?PersonUser['.$model->LoginField.']='.$model->{$model->LoginField}.'&PersonUser[activation_code]='.$model->activation_code;
		$this->_data = array(
			'{email}'=>$model->email,
	...
	protected function seturlaction($message){
		switch ($message) {
			case self::ACTIVATION:
				return 'activate';
				break;
...
class UGMailActivation implements UGMailMessage {
	...
	public function mailSubject($data)
	{
		return Yii::t('PersonModule.mail', 'account activation');
	}
	...
	public function mailSuccess($data)
	{
		return Yii::t('PersonModule.general','The activation email was successfully sent to {email}', $data);
	}
	...
	public function mailError($data)
	{
		return Yii::t('PersonModule.general','Impossible to send email to the address {email}', $data);
	}
}
...
class UGMailPassReset implements UGMailMessage {
	...
	public function mailSubject($data)
	{
		return Yii::t('PersonModule.mail', 'password reset request');
	}
	...
	public function mailSuccess($data)
	{
		return Yii::t('PersonModule.general','An email containing the instructions to reset your password has been sent to your email address: {email}', $data);
	}
...
class UGMailInvitation implements UGMailMessage {
	...
	public function mailSubject($data)
	{
		return Yii::t('PersonModule.mail', 'invitation to {website}', $data);
	}
	...
	public function mailSuccess($data)
	{
		return Yii::t('PersonModule.general','An invitation email was sent to the address {email}', $data);
	}
