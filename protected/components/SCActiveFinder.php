...
/**
 * SCActiveFinder implements eager loading and lazy loading of related active records.
 *
 */
class SCActiveFinder extends CActiveFinder
{
	 ...
	 * @param SCActiveRecord $model the model that initiates the active finding process
	...
	public function __construct($model,$with)	{
		...
		$this->_joinTree=new SCJoinElement($this,$model);
	 ...
	 * @param SCActiveRecord $baseRecord the base record whose related objects are to be loaded
	 */
	 ...
	 * @param SCJoinElement $parent the parent tree node
	...
	protected function buildJoinTree($parent,$with,$options=null)
	{
		...
		if(is_string($with))
		{
			...
			$model=SCActiveRecord::model($relation->className);
			...
			else
			{
				if(isset($parent->children[$with]))
				{
					...
					$element=new SCJoinElement($this,$relation,$parent,++$this->_joinCount);
...
/**
 * SCJoinElement represents a tree node in the join tree created by {@link CActiveFinder}.
 */
class SoCJoinElement extends CJoinElement
{
	...
	{
		Yii::trace('in count','SCJoinElement');
		$query=new CJoinQuery($this,$criteria);
		$this->_finder->baseLimited=false;
		$this->_finder->joinAll=true;
		$this->buildQuery($query);
		
		$diff = array_diff($query->groups, array(0,''));  //lsd
		$notScalar = (!empty($diff)) ? true : false; //lsd
		
		$selectnoscalar = is_array($query->selects) ? implode(',',$query->selects) : $criteria->select; //lsd

		$select=is_array($criteria->select) ? implode(',',$criteria->select) : $criteria->select;
		
		if($select!=='*' && !strncasecmp($select,'count',5))
			$query->selects=$notScalar ? array($selectnoscalar) : array($select); // lsd
		else if(is_string($this->_table->primaryKey))
		{
			$prefix=$this->getColumnPrefix();
			$schema=$this->_builder->getSchema();
			$column=$prefix.$schema->quoteColumnName($this->_table->primaryKey);
			$query->selects=$notScalar ? array("COUNT(DISTINCT $column), ".$selectnoscalar) : array("COUNT(DISTINCT $column)"); // lsd
		}
		else
			$query->selects=array("COUNT(*)");

		//$query->orders=$query->groups=$query->havings=array();
		$query->orders=$query->havings=array(); // lsd
		
		$query->limit=$query->offset=-1;
		$command=$query->createCommand($this->_builder);

		if ($notScalar) return $command->queryAll(true); //lsd
		return $command->queryScalar();
	}
}

class SCJoinElement extends CJoinElement
{
	/**
	...
	public $slave;
	/**
	 * @var SCActiveRecord the model associated with this tree node
	 */
	public $model;
	/**
	...
	/**
	 * Constructor.
	 * @param SCActiveFinder $finder the finder
	...
	public function __construct($finder,$relation,$parent=null,$id=0)
	{
		...
		if($parent!==null)
		{
			...
			$this->_parent=$parent;
			$this->model=SCActiveRecord::model($relation->className);
	...
	public function find($criteria=null)
	{
		...
		{
			$query=new SCJoinQuery($this,$criteria);
		...
		{
			$query=new SCJoinQuery($this->_parent);
			$this->_joined=true;
			$query->join($this);
			$this->buildQuery($query);
	 ...
	 * @param SCActiveRecord $baseRecord the active record whose related object is to be fetched.
	 */
	public function lazyFind($baseRecord)
	{
	 ...
	 * @param SCJoinQuery $query represents a JOIN SQL statements
	 * @param SCActiveRecord $record the active record whose related object is to be fetched.
	 */
	 ...
	 * @return string number of primary records. Note: type is string to keep max. precision.
	 */
	public function countt($criteria=null)
	{
		$query=new SCJoinQuery($this,$criteria);
	...
	/**
	 * Calls {@link SCActiveRecord::beforeFind}.
	...
	/**
	 * Calls {@link SCActiveRecord::afterFind} of all the records.
	 */
	 ...
	 * @param SCJoinQuery $query the query being built up
	 */
	 ...
	 * @param SCJoinQuery $query the query to be executed.
	 */
	 ...
	 * @param SCJoinQuery $query the query executed
	 * @param array $row a row of data
	 * @return SCActiveRecord the populated record
	 */
	private function populateRecord($query,$row)
	{
		...
		foreach($this->children as $child)
		{
			...
			else // has_many and many_many
			{
				...
				{
					if($childRecord instanceof SCActiveRecord && $child->relation->index!==null)
	 ...
	 * @param SCJoinElement $fke the join element containing foreign keys
	 * @param array $fks the foreign keys
	 * @param SCJoinElement $pke the join element containg primary keys
	 * @param SCJoinElement $parent the parent join element
...
/**
 * SCJoinQuery represents a JOIN SQL statement.
 *
 */
class SCJoinQuery extends CJoinQuery
{
	/**
	...
	/**
	 * Constructor.
	 * @param SCJoinElement $joinElement The root join tree.
	 * @param CDbCriteria $criteria the query criteria
	 */
 ...
 */
class SCStatElement extends CStatElement
{
	/**
	 * @var CActiveRelation the relation represented by this tree node
	 */
	 ...
	 * @param SCJoinElement $parent the join element owning this STAT element
	 */
